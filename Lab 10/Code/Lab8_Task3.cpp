...
HashTable<Student>* getHashChoice(); // Get a hash choice given a specific menu. Returns the specified hash table

int main () {
    // Get the hash based on menu selection
    // Makes classRoom either a normal or chained hash
    HashTable <Student>* classRoom = getHashChoice();
    ...
    do{
        ...
        cout<<"3: Remove a student from the list"<<endl;
        cout<<"4: Check how many students are in the list"<<endl;
        ...
        try{
            switch (menu){
                ...
                case 2:
                    {
                        ...
                        if(temp == nullptr){
                            ...
                            cout<<"Found student: "<<temp->getName()<<endl;
                        }
                    }
                    break;
                case 3:
                    {
                        Student *temp=classRoom->removeItem(new Student("","",getMNumber(),""));
                        if (temp == nullptr){
                            cout<<"That student is not in the list"<<endl;
                        }
                        else{
                ...
                case 4:
                    {
                        int length = classRoom->getLength();
                        if(length == 0){
                            cout<<"The table is empty"<<endl;
                        }
                        else{
                            cout<<"The table contains " << length << " student(s)"<<endl;
                        }
                    }
                    break;
            }
...
HashTable<Student>* getHashChoice(){
    cout << "What hash table should we use?" << endl;
    cout << "1. Normal hash table" << endl;
    cout << "2. Chained hash table" << endl;

    int choice = checkInt("Enter your selection: ", 1, 2);

    HashTable<Student>* returnVal = choice == 1 ? new HashTable<Student> : new HashChain<Student>;
}
